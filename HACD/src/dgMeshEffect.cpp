	...
	:dgPolyhedra (mesh) 
{
	...
	m_points = (dgBigVector*) HACD_ALLOC(int32_t (m_maxPointCount * sizeof(dgBigVector)));
	...
	m_attib = (dgVertexAtribute*) HACD_ALLOC(int32_t (m_maxAtribCount * sizeof(dgVertexAtribute)));
...
void dgMeshEffect::ApplyAttributeArray (dgVertexAtribute* const attib, int32_t maxCount)
{
	...
	HACD_FREE (m_attib);
	m_attib = (dgVertexAtribute*)HACD_ALLOC(int32_t (m_atribCount * sizeof(dgVertexAtribute)));
...
void dgMeshEffect::AddAtribute (const dgVertexAtribute& attib)
{
	if (m_atribCount >= m_maxAtribCount) {
		m_maxAtribCount *= 2;
		dgVertexAtribute* const attibArray = (dgVertexAtribute*) HACD_ALLOC(int32_t (m_maxAtribCount * sizeof(dgVertexAtribute)));
...
void dgMeshEffect::AddVertex(const dgBigVector& vertex)
{
	if (m_pointCount >= m_maxPointCount) {
		m_maxPointCount *= 2;
		dgBigVector* const points = (dgBigVector*) HACD_ALLOC(int32_t (m_maxPointCount * sizeof(dgBigVector)));
...
void dgMeshEffect::PackVertexArrays ()
{
	if (m_maxPointCount > m_pointCount) {
		dgBigVector* const points = (dgBigVector*) HACD_ALLOC(int32_t (m_pointCount * sizeof(dgBigVector)));
		memcpy (points, m_points, m_pointCount * sizeof(dgBigVector));
		HACD_FREE(m_points);
		m_points = points;
	...
	if (m_maxAtribCount > m_atribCount) {
		dgVertexAtribute* const attibArray = (dgVertexAtribute*) HACD_ALLOC(int32_t (m_atribCount * sizeof(dgVertexAtribute)));
		memcpy (attibArray, m_attib, m_atribCount * sizeof(dgVertexAtribute));
		HACD_FREE(m_attib);
		m_attib = attibArray;
		m_maxAtribCount = m_atribCount;
...
void dgMeshEffect::AddPolygon (int32_t count, const float* const vertexList, int32_t strideIndBytes, int32_t material)
{
	...
	int32_t stride = strideIndBytes / sizeof (float);
	for (int32_t i = 0; i < count; i ++) 
	{
...
dgMeshEffect::dgIndexArray* dgMeshEffect::MaterialGeometryBegin()
{
	...
	dgIndexArray* const array = (dgIndexArray*) HACD_ALLOC (int32_t (4 * sizeof (int32_t) * GetCount() + sizeof (dgIndexArray) + 2048));
	array->m_indexList = (int32_t*)&array[1];
...
//		m_parallerConcavityCalculator.SetThreadsCount(DG_CONCAVITY_MAX_THREADS);
		...
		for (iter.Begin(); iter; iter++) {
			...
			if ((edge->m_mark != meshMask) && (edge->m_incidentFace > 0)) {
				...
				do {
					...
					ptr->m_userData = uint64_t (clusterNode);
					ptr->m_mark = meshMask;
					ptr = ptr->m_next;
				} while (ptr != edge);
	...
	static float RayHit (void *context, const float* const polygon, int32_t strideInBytes, const int32_t* const indexArray, int32_t indexCount)
	{
		...
		if (t < me.m_param) {
			int32_t faceColor = me.m_me->GetTagId(indexArray);
		...
		for (dgListNode* node = GetFirst(); node; node = node->GetNext()) {
			...
			do {
				...
				if ((edge->m_mark != meshMask) && (twinFaceIndex != faceIndex) && (twinFaceIndex > 0)) {
					...
					edge->m_userData = uint64_t(pairNode);
					edge->m_twin->m_userData = uint64_t(pairNode);
