...
int32_t dgVertexListToIndexList (float* const vertList, int32_t strideInBytes, int32_t floatSizeInBytes, int32_t unsignedSizeInBytes, int32_t vertexCount, int32_t* const indexList, float tolerance)
{
	...
	int32_t stride = strideInBytes / sizeof (float);

	HACD_ASSERT (!unsignedSizeInBytes);
	dgStack<double> pool(vertexCount * stride);

	int32_t floatCount = floatSizeInBytes / sizeof (float);
	...
	int32_t count = dgVertexListToIndexList (data, stride * sizeof (double), floatCount, vertexCount, indexList, double (tolerance));
	for (int32_t i = 0; i < count; i ++) {
